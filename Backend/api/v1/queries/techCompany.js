let table_name = "mtae_techs";


let getAllTechList = () => {
    
    return `SELECT ${table_name}.id, ${table_name}.name,${table_name}.ratings,${table_name}.completed_case,${table_name}.description, ${table_name}.email, ${table_name}.phone, ${table_name}.fax, ${table_name}.address,
     ${table_name}.license, ${table_name}.cover_image, ${table_name}.profile_image,${table_name}.status,
    ...
    WHERE mtae_users.role_id = 6 and ${table_name}.status > 0
    order by ${table_name}.id ASC 
    `;
} 


let getActiveTechList = () => {
    ...
    mtae_zipcodes ON
    (mtae_zipcodes.id = ${table_name}.zip_id)
    WHERE mtae_users.role_id = 6 and ${table_name}.status = 1
    order by ${table_name}.ratings DESC
    `;
} 

let getTechListByName = () => {
    ...
    mtae_users.role_id = 6 and ${table_name}.status = 1
    order by ${table_name}.id ASC 
    `;
} 


let getAllTechListWithLimit = () => {
...
let getNextTechList = () => {
    ...
    mtae_zipcodes ON
    (mtae_zipcodes.id = ${table_name}.zip_id)
    WHERE mtae_users.role_id = 6 and ${table_name}.id > ? and ${table_name}.status > 0
    order by ${table_name}.id ASC
    LIMIT  ?`;
} 


let getTechCompanyById = () => {
