  ...
  <div class="row">
    <div class="col-lg-12">
      <table
        ...
        id="caseTbl"
        v-if="this.deletedCaseList.length > 0"
        style="width: 100%"
      >
        <thead>
          <tr>
            <th class="tableHeader" align="center">Case ID</th>
            <th class="tableHeader" align="center">Patient Name</th>
            <th class="tableHeader" align="center">Deleted Date</th>
            <th class="tableHeader" align="center">Actions</th>
          </tr>
        </thead>
        <tbody class="mx-lg-2">
          <tr v-for="(item, index) in deletedCaseList" :key="index">
            ...
            <td class="normalFont" align="center" aria-label="Action">
              <button
                class="btn-restore btn-md"
                @click.prevent="recoverCase(item.id)"
              >
      ...
      response => {
        ...
        if (response.data.data) {
          response.data.data.map(e => {
            if (e.type === 'Case') {
              this.deletedCaseList = e.data.length > 0 ? e.data : [];
              this.BasePath = e.folder;
            }
            // if (this.deletedCaseList) {
            setTimeout(() => {
              $('#caseTbl').DataTable({
                lengthMenu: [
                  [10, 25, 50, -1],
            ...
            // console.log(this.deletedCaseList);
            // }
          });
        }
      },
      ...
      trashAPI.recoverCase(id).then(
        response => {
          if (response.data.success === true) {
            ...
            this.deletedCaseList = this.deletedCaseList.filter(
              e => e.id !== id,
            );
          } else {
            createToast(response.data.message, {
