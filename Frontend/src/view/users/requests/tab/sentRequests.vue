  ...
  <div class="row" id="mtSR">
    ...
    <div class="col-lg-9 px-0 px-lg-auto pr-lg-2 full">
      <div v-if="this.recommendedData.length > 0">
        <table class="table tableBorder" style="width: 100%" id="datatable2">
          <thead>
            <tr>
              ...
              <th class="tableHeader" align="center" width="150px">Status</th>
              <th class="tableHeader" width="120px" align="center">Actions</th>
            </tr>
          </thead>
          <tbody class="mx-lg-2">
            <tr v-for="(item, index) in recommendedData" :key="index">
              ...
              <!-- <td class="normalFont" align="center" aria-label="Patient Name">
                {{ item.patient_name }}
              </td> -->
              <td
                class="normalFont"
              ...
              <td class="normalFont" align="center" aria-label="Status">
                ...
                <span class="text-danger font-weight-bold">
                  {{ item.service_provider_response === 2 ? 'Canceled' : '' }}
                </span>
                <span class="text-dark font-weight-bold">
                ...
                <br />
                <span v-if="item.service_provider_response === 3" class="small">
                  <!-- {{
                    '(Auto Cancel ' +
                    moment.utc(item.expired_at).endOf('day').fromNow() +
                    ')'
                  }} -->
                  {{
                    '(Auto Cancel ' +
                  ...
                  <!-- {{
                    moment(item.expired_at).add(6, 'hours')
                  }}
                  <br/>
                  {{
                    item.expired_at
              ...
              <td class="normalFont" align="center" aria-label="Action">
                ...
                >
                  <i class="feather icon-x"></i> <span>Cancel</span>
                </button>
                <p v-else class="action-not-available">-</p>
              </td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    // cancel Request
    cancelRequest(id) {
      requestsAPI.dentistCancelSentRequest(id).then(
        response => {
    ...
    getSentRequestList() {
      requestsAPI.dentistSentRequestList().then(
        response => {
          if (response.data.success === true) {
            this.recommendedData = response.data.data;
            this.loading = false;
    ...
    resetSearch() {
      ...
      $('#datatable2').DataTable().column(3).search('').draw();
    },
    ...
    caseFilter() {
      ...
      if (this.serviceType) {
        $('#datatable2').DataTable().column(2).search(serviceType).draw();
      }
...
/* @import url('//cloud.webtype.com/css/310d85fe-2595-4325-b12b-7efc76721ac9.css'); */
