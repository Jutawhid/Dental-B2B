      ...
      <div class="content-wrapper">
        <div class="row">
          <div class="col-12 col-sm-12">
            <h3 class="text-bold-700 mb-0 headerTXT">Invoice Details</h3>
          </div>
          <div class="col-12">
            <hr />
          </div>
                  ...
                  <div class="mh">
                    <table class="inv-tbl my-2">
                      ...
                      <tbody v-for="(item, index) in items" :key="index">
                        ...
                        <tr class="final">
                          <td>
                            <span class="inv-title">Case ID :</span>
                            <span class="inv-title">
                              {{ items.map(e => e.caseId).toString() }}</span
                            >
                          </td>
                          <td>
                            <span class="inv-title">Service Status :</span>
                            <span
                              class="inv-title"
                              v-if="
                                items.map(e => e.serviceStatus).toString() ===
                                '1'
                              "
                            >
                              To-Do
                  ...
                  <div class="inv-header final p-0">
                    <div class="d-flex col-12 px-0">
                      <div class="col-4">
                        ...
                        <p class="footer-title sby">Signed By</p>
                      </div>
...
export default {
  ...
  mounted() {
    ...
    this.heading = 'InvoiceDetails#INV' + this.invoiceId;
    setTimeout(() => {
  ...
  methods: {
    ...
    getInvoiceDetails() {
      ...
      if(this.currentUser.role.role_id === 1 || this.currentUser.role.role_id === 2){
        if (this.invoiceId) {
        ...
        if (this.invoiceId) {
        paymentAPI.getInvoiceDetails(this.invoiceId).then(
          response => {
            if (response.data.data) {
              this.items = response.data.data.map(e => {
                return {
                  serviceNo: 'D-' + e.service.id,
                  serviceStatus: e.service.active_status,
                  details: e.service_name,
                  total: '$ ' + e.amount,
                  created_at: e.created_at,
                  caseId: e.case_details.case_id,
    ...
    downloadWithCSS() {
      const columns = [
        ...
        { title: 'Service Name', dataKey: 'details' },
      ...
      var imgData = 'https://iili.io/VhTOuf.png';
      require('jspdf-autotable');
      // text is placed using x, y coordinates
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(16).text('Invoice : #INV' + this.invoiceId, 1, 1);

      doc.addImage(imgData, 'JPG', 6, 0.7, 1.5, 0.3);

      doc.setFontSize(12).text('Case ID : '+ this.items.map(e => e.caseId).toString(), 1, 2.5);
      doc.setFontSize(12).text( 'Total = '+this.items.map(e => e.total).toString(), 6, 2.5);

      doc.autoTable({
        columns,
        body: this.items,
        margin: { left: 1, top: 1.5, right: 1, bottom: 1 },
      });
      //
