            ...
            <div class="row">
              <div class="col-6">
                ...
                <select
                  ...
                  class="form-control"
                  @blur="validate('service_id')"
                  @keypress="validate('service_id')"
                >
                ...
                <p class="form-input-hint" v-if="!!errors.service_id">
                  {{ errors.service_id }}
                </p>
              </div>
              <div class="col-6">
                ...
                />
                <p class="form-input-hint" v-if="!!errors.price">
                  {{ errors.price }}
                </p>
              </div>
...
export default {
  ...
  created() {
    ...
    // console.log(this.$store.state.profile.user.role.role_id);
  },
  data() {
    const schema = object().shape({
      ...
      service_id: string().required('This is required field'),
      price: string().required('This is required field'),
    });
    return {
      schema,
      role_id: '',
  ...
  methods: {
    ...
    onSubmit() {
      if (this.service.service_id === '') {
        this.errors.service_id = 'Service type is required';
      }
      ...
      if (
        ...
        userAPI.addProfileService(this.service).then(
          res => {
            if (res.data.success == true) {
              if (
                this.role_id === 5 ||
                ...
                this.$router.push('/');
                createToast('User profile not found', {
                  position: 'top-right',
                  type: 'danger',
                  transition: 'bounce',
                  showIcon: 'true',
