  ...
  <div class="row">
    ...
    <div class="col-lg-4 d-flex">
      <input
        ...
        v-model="searchTrash"
        @change="searchingTrash()"
      />

      <i
        v-if="!this.searchTrash"
        class="feather icon-search searchTrashIcon searchBtn"
        @click="searchingTrash()"
      ></i>
      <i
        v-if="this.searchTrash"
        class="feather icon-x searchTrashIcon searchBtn"
        @click="clearSearch()"
      ></i>
    </div>
    <div class="col-lg-12 mt-2">
        ...
        <tbody class="mx-lg-2">
          <tr v-for="(item, index) in deletedFileList" :key="index">
            ...
            <td
              ...
              align="center"
              aria-label="Deleted By"
            >
              {{ item.deleted_by + '(' + item.role + ')' }}
            </td>
      ...
      <p
        ...
        v-if="!loading && this.deletedFileList.length === 0"
      >
        Sorry, No data found.
      </p>
    </div>
...
export default {
  mounted() {
    trashAPI.getAdminTrashList().then(
      response => {
        // console.log(response);
        if (response.data.data) {
          this.deletedFileList = response.data.data;
          this.BasePath = response.data.folder;
          this.loading = false;
          setTimeout(() => {
  ...
  methods: {
    ...
    downloadFile(id, file_name) {
      trashAPI.downloadAdminTrashFile(id).then(
        response => {
    ...
    // restore File
    restoreFile(id) {
      trashAPI.restoreAdminTrashFile(id).then(
        response => {
          // console.log(response);
    ...
    searchingTrash() {
      if (this.searchTrash) {
        let search = this.searchTrash.toUpperCase();
