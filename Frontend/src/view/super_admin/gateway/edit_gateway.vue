      ...
      <div class="content-wrapper">
        <div class="row">
          <div class="col-lg-4 my-1">
            <h1 class="text-bold-700 mb-0 headerTXT">Edit Gateway</h1>
          </div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      selectedGateway: this.$route.params,
    };
  },
  mounted() {
    console.log(this.$route.params);
    // if (this.$route.params.id) {
    this.selectedGateway = this.$route.params.id
      ? Number(this.$route.params.id)
      : null;
    // }
    if (this.selectedGateway) {
      userAPI.getGatewayDetails(this.selectedGateway).then(
        response => {
  ...
  methods: {
    ...
    onSubmit(values) {
      let userData = {
        id: this.selectedGateway ? this.selectedGateway : null,
        name: values.name ? values.name : null,
        currentImage: this.userData.currentImage
          ? this.userData.currentImage
          : '',
      };

      this.progress = 0;
      uploadProfile
        .editGateway(userData, event => {
